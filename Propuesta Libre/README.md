...
1. El cliente inicia desde **WhatsApp** o **sitio web**.
2. El sistema autentica al cliente (por número de teléfono o email).
3. Se le presentan **emojis predeterminados** para indicar su nivel de molestia.
4. El cliente ingresa el ID del pedido y adjunta foto (si aplica).
5. El sistema valida, deduplica y procesa igual que en el flujo original, pero con **trazabilidad visible para el cliente** (puede consultar estado en cualquier momento).
6. Si el supervisor debe aprobar, se le envía un mensaje enriquecido a un canal interno de **Slack/Teams** con el contexto y la foto.
7. El cliente recibe notificaciones por el mismo canal (WhatsApp o web).
...
| **Captura de sentimiento** | No estructurada (requiere NLP) | Estructurada con emojis, sin costo adicional |
...
**Justificación:** El esfuerzo se amortiza rápidamente al reducir el trabajo manual de los agentes.
